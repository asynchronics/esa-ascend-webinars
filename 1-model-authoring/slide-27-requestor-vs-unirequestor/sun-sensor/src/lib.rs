...
pub struct SunSensor {
    ...
    pub sun_position_req: Requestor<(), [f32; 3]>,
}
...
impl SunSensor {
    ...
    pub async fn tc(&mut self, cmd: SampleCommand) {
        ...
        let sun_position = match self.sun_position_req.send(()).await.next() {
            Some(pos) => pos,
            None => {
                println!("Sun position could not be retrieved");
                return;
            }
        };
    ...
    pub fn new(address: u8) -> Self {
        Self {
            ...
            voltage: 0.0,
            tm: Output::default(),
            sun_position_req: Requestor::default(),
        }
    }
}
